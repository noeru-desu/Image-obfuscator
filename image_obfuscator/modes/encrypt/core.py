...
from image_obfuscator.modules.image import array_to_image, random_noise
from image_obfuscator.pregenerated.image_encrypt import (FlipFuncV1, FlipFuncV2,
                                                        MappingFuncV1,
                                                        MappingFuncV2,
                                                        MappingFuncV3)
...
class Shuffle(object):
    ...
    def obverse(self, x: MutableSequence, reset_seed: bool = True) -> list:
        new_x = x.copy() if hasattr(x, 'copy') else copy(x)
        return self.obverse_on_self(new_x, reset_seed)

    def reverse(self, x: MutableSequence, reset_seed: bool = True) -> list:
        new_x = x.copy() if hasattr(x, 'copy') else copy(x)
        return self.reverse_on_self(new_x, reset_seed)

    def obverse_on_self(self, x: MutableSequence, reset_seed: bool = True):
        with self._lock:
            if reset_seed:
                random.seed(self.seed)
            random.shuffle(x)
    ...
    def reverse_on_self(self, x: MutableSequence, reset_seed: bool = True):
        with self._lock:
            if reset_seed:
                random.seed(self.seed)
            randbelow = random._randbelow
