...
class Timeit(object):
    __slots__ = (
        'func', 'func_name', 'total_running_time', 'average_time', 'running_times', 'bound',
        'default_args', 'repeat_times', 'prompt', 'print_each_times'
    )

    def __init__(self, *default_args, repeat_times: int = 1, print_each_times: bool = False, prompt: bool = True) -> None:
        assert repeat_times > 0, '"repeat_times" must be greater than 0.'
        self.default_args = default_args
        self.repeat_times = repeat_times
        self.print_each_times = print_each_times
        self.prompt = prompt
    ...
    def __call__(self, func: FunctionType):
        ...
        def wrap(*args, **kwargs):
            for i in range(1, self.repeat_times + 1):
                ...
                if self.print_each_times and self.repeat_times != 1:
